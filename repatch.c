...
int checkFile(const char *filename) {
	SceIoStat k_stat;
	return !sceIoGetstat(filename, &k_stat);
}
...
static int overlayHandler(uint32_t pid, mount_point_overlay *overlay_old, mount_point_overlay *overlay_new, int opt) {
	...
	char titleid[32];
	if(sceKernelGetProcessTitleId(pid, titleid, sizeof(titleid))==0) {
		if(opt & APP_PATH)
...
static int sceFiosKernelOverlayAddForProcessForDriver_patched(uint32_t pid, mount_point_overlay *overlay, uint32_t *outID) {
	...
	if(!sceSblACMgrIsSystemProgram(pid)) { // Damn. I just enabled most homebrew support with FIOS2
...
static int sceFiosKernelOverlayResolveSyncForDriver_patched(SceUID pid, int resolveFlag, const char *pInPath, char *pOutPath, size_t maxPath) {
	...
	if(!sceSblACMgrIsSystemProgram(pid)) { // Damn. I just enabled most homebrew support with FIOS2
		if(strstr(pOutPath, "ux0:/data")) {
			...
			if (ret)
				strncpy(pOutPath, data_patch, PATH_MAX);
		}
	}
	return ret;
}
	...
	if ((flag & SCE_O_WRONLY) != SCE_O_WRONLY && sceSblACMgrIsSceShell(0) && (strncmp(filename, "ux0:", sizeof("ux0:") -1) == 0) && strstr(filename, "/eboot.bin") != NULL){
			...
			if((ret = TAI_CONTINUE(int, ref_hooks[1], eboot_path, flag, mode))>0) {
				...
				*end_path = 0;
				...
				SceUID fd = sceIoOpen(eboot_path, SCE_O_RDONLY, 0);
				if (fd >= 0) {
					if (sceIoRead(fd, &self_auth_info, 0x90) != 0x90)
						memset(&self_auth_info, 0, sizeof(self_auth_info));
					sceIoClose(fd);
				}
				if (hooks_uid[6] <= 0)
					hooks_uid[6] = taiHookFunctionImportForKernel(KERNEL_PID, &ref_hooks[6], "SceKernelModulemgr", TAI_ANY_LIBRARY, 0xF3411881, sceSblAuthMgrAuthHeaderForKernel_patched);
			}
	}
...
static int sceAppMgrDrmOpenForDriver_patched(drm_opts *drmOpt, int r2) {
	...
	if(ret < 0 && !sceSblACMgrIsSceShell(0))
...
static int sceAppMgrGameDataMount_patched(char *input, int r2, int r3, char *outpath) {
	int ret = -1;
...
int module_start(SceSize argc, const void *args) {
	...
	hooks_uid[1] = taiHookFunctionImportForKernel(KERNEL_PID, &ref_hooks[1], "SceKernelModulemgr", TAI_ANY_LIBRARY, 0x75192972, sceIoOpen_patched);
